  ...
  async main(ctx) {
    ...
    document.addEventListener('pointerup', (e) => {
      ...
      if (selClickInfo) {
        ...
        // Guard: if pointer didn't move (click, not drag) and the selection
        // contains multiple words, it was likely auto-generated by CSS
        // user-select:all — skip and let the single-click handler deal with it.
        const clickDragDist = Math.abs(e.clientX - pointerDownPos.x) + Math.abs(e.clientY - pointerDownPos.y);
        ...
        // isFakeSelection: fall through to drag-select handler below
      }
