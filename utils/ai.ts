...
3. 对于「头衔 + 人名」的组合（如 US President Donald Trump），必须作为一个整体提取。
4. 提取的原文必须与段落中的文本**精确匹配**（大小写、单复数、时态一致），以便前端能精准定位和注入标注。
5. 对于专有名词（人名、机构名、地名、专有事件），请在第三列提供有价值的百科式背景介绍（10-30字以内）。对于普通词汇，第三列必须留空。
6. 对于纯数字、年份，不要翻译，直接在第三列提供英文拼写读法。
7. 绝对不要输出音标！音标由前端引擎独立处理。
